...
if __name__ == '__main__':
    ...
    parser.add_argument('--interval_max', default=32, type=int)
    ...
    sampler = WarpSampler(user_train, usernum, itemnum, relation_matrix, UsersItem_diff, batch_size=args.batch_size,
                          maxlen=args.maxlen,
                          n_workers=3)
    ...
    for epoch in range(epoch_start_idx, args.num_epochs + 1):
        ...
        for step in range(num_batch):  # tqdm(range(num_batch), total=num_batch, ncols=70, leave=False, unit='b'):
            u, seq, time_seq, time_matrix, pos, neg, time_diff = sampler.next_batch()  # tuples to ndarray
            u, seq, pos, neg, time_diff = np.array(u), np.array(seq), np.array(pos), np.array(neg), np.array(time_diff)
            time_seq, time_matrix = np.array(time_seq), np.array(time_matrix)
            pos_logits, neg_logits = model(u, seq, time_matrix, pos, neg, time_diff)
        ...
        if epoch == args.num_epochs:
            ...
            fname = 'TiSASRec.epoch={}.lr={}.layer={}.head={}.hidden={}.maxlen={}.alpha={}.pth'
