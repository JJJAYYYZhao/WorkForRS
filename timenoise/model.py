...
class TimeAwareMultiHeadAttention(torch.nn.Module):
    ...
    def __init__(self, hidden_size, head_num, dropout_rate, dev, alpha):
    ...
    def forward(self, queries, keys, time_mask, attn_mask, time_matrix_K, time_matrix_V, abs_pos_K, abs_pos_V,
        ...
        noiseFigure_length = time_diff.shape[0]
        # print(time_diff.shape)
        # print(noiseFigure_length)
        # print(self.hidden_size)
        # padding_length = self.hidden_size - noiseFigure_length
        # if padding_length > 0:
        #     padding = np.zeros(padding_length)
        #     time_diff = np.concatenate(padding, time_diff)
        #     # print(time_diff.shape[0])
        # else:
        #     # 事实上，time_diff长度比hidden_size长，因此要取样
        #     time_diff = time_diff[:][-self.hidden_size:]
        #     # print(time_diff.shape)
        #     # print(1)
        # print(time_diff.shape)
        # time_diff = torch.LongTensor(time_diff)
        # time_diff = torch.unsqueeze(time_diff, dim=1)
        # print(attn_weights.shape)
        ...
        time_noise_figure = self.softmax(time_noise_figure)
        attn_weights = (1 - self.alpha) * attn_weights + self.alpha * time_noise_figure
...
class TiSASRec(torch.nn.Module):  # similar to torch.nn.MultiheadAttention
    ...
    def seq2feats(self, user_ids, log_seqs, time_matrices, time_diff):
    ...
    def forward(self, user_ids, log_seqs, time_matrices, pos_seqs, neg_seqs, time_diff):  # for training
        log_feats = self.seq2feats(user_ids, log_seqs, time_matrices, time_diff)
    ...
    def predict(self, user_ids, log_seqs, time_matrices, item_indices, time_diff):  # for inference
        log_feats = self.seq2feats(user_ids, log_seqs, time_matrices, time_diff)
